    ...
    if not os.path.exists(mmapdir+'/'+infoname) or True:
        if use_glymur:
            ...
            with TicToc('expand time') as expandtime:
                ...
                ret = subprocess.call(f'LD_LIBRARY_PATH=./lib ./kdu_expand -num_threads 64 -i {jp2path} -o {tmpoutname}', shell=True)
                if ret!=0:
